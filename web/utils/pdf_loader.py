    ...
    def _wordlist_to_text(self, word_list, page_width, text_kwargs=None):
        ...
        for word in word_list[1:]:
            ...
            if ((word_x1 - text_kwargs['gap_tolerance'] <= word['x0'] <= word_x1 + text_kwargs['gap_tolerance']
                ...
                if is_bold:
                    sent_str += '*'
                sentence_list.append(re.sub(r' +', ' ', re.sub(r'( *\t+ *)+', '\t', sent_str.rstrip().replace('\xa0', ' '))))
    ...
    def _convert_images_to_words_from_page(self, page: pdfplumber.page.Page) -> list:
        ...
        for img in page.images:
            if img["stream"]["Filter"].name in _PDF_FILTER_WITHOUT_LOSS:
                images.append(
                    [Image.fromarray(np.frombuffer(img["stream"].get_data(), dtype=np.uint8).reshape(img["stream"]["Height"], img["stream"]["Width"], -1)).convert('RGB'), img]
                )
        ...
        for image_tuple in images:
            # normalize image background color to white
        ...
        for image_tuple in filted_images:
            ...
            # get every non-1 (= non-background) segment and its pos
            segment_dict_list = []
            is_bg = True
            ...
            """
            # crop the image and append results in dict form
            accum_height = 0
            pos_per_pixel = (image_tuple[1]['bottom'] - image_tuple[1]['top']) / image_tuple[0].height
            for crop_height in crop_height_list:
                image_dict_list.append({'image': image_tuple[0].crop((0, accum_height, image_tuple[0].width, crop_height)),
                                        'x0': image_tuple[1]['x0'],
                                        'x1': image_tuple[1]['x1'],
